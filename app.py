...
st.set_page_config(page_title='Javna Nabava V14', layout='wide')
st.markdown("""<style>
...
def ask_delete(i):
    st.session_state.confirm_id = i

def do_delete():
    delete_tender(st.session_state.confirm_id)
    st.session_state.confirm_id = None
...
if mode == 'Admin unos':
    ...
    with st.form('add_form', clear_on_submit=True):
        ...
        with right:
            doc = st.selectbox('Vrsta dokumenta', [
                'Odaberi', 'Bagatelna nabava', 'Izmjena dokumentacije za nadmetanje',
                'Obavijest o početku postupka javne nabave', 'Obavijest o rezultatima natječaja',
                'Obavijest o sklopljenim ugovorima', 'Objava o poništenju nadmetanja / ispravku objave',
                'Objava o uspostavljanju kvalifikacijskog sustava', 'Pojednostavljena obavijest u dinamičkom sustavu nabave',
                'Poziv na nadmetanje', 'Poziv na natječaj', 'Prethodna (indikativna) obavijest', 'PRODAJA'
            ], key='add_doc')
            ug = st.selectbox('Vrsta ugovora', [
                'Odaberi', 'Isporuka robe', 'Javne usluge', 'Javni radovi', 'PRODAJA'
            ], key='add_ug')
            post = st.selectbox('Vrsta postupka', [
                'Odaberi', 'Bagatelna nabava', 'Natječaj', 'Ograničeni postupak', 'Otvoreni postupak',
                'Pregovarački postupak bez prethodne objave', 'Pregovarački postupak s prethodnom objavom',
                'PRODAJA', 'Ugovori s povezanim Društvima', 'Usluge iz Dodatka II.B'
            ], key='add_post')
    ...
    if st.session_state.edit_id is not None:
        ...
        with st.form('edit_form', clear_on_submit=True):
            ...
            with r:
                edoc = st.selectbox('Vrsta dokumenta', [
                    'Odaberi', 'Bagatelna nabava', 'Izmjena dokumentacije za nadmetanje',
                    'Obavijest o početku postupka javne nabave', 'Obavijest o rezultatima natječaja',
                    'Obavijest o sklopljenim ugovorima', 'Objava o poništenju nadmetanja / ispravku objave',
                    'Objava o uspostavljanju kvalifikacijskog sustava', 'Pojednostavljena obavijest u dinamičkom sustavu nabave',
                    'Poziv na nadmetanje', 'Poziv na natječaj', 'Prethodna (indikativna) obavijest', 'PRODAJA'
                ], index=0, key='edit_doc')
                eug = st.selectbox('Vrsta ugovora', [
                    'Odaberi', 'Isporuka robe', 'Javne usluge', 'Javni radovi', 'PRODAJA'
                ], index=0, key='edit_ug')
                epost = st.selectbox('Vrsta postupka', [
                    'Odaberi', 'Bagatelna nabava', 'Natječaj', 'Ograničeni postupak', 'Otvoreni postupak',
                    'Pregovarački postupak bez prethodne objave', 'Pregovarački postupak s prethodnom objavom',
                    'PRODAJA', 'Ugovori s povezanim Društvima', 'Usluge iz Dodatka II.B'
                ], index=0, key='edit_post')
            new_files = st.file_uploader('Novi prilozi', accept_multiple_files=True, key='edit_files')
            sb, cb = st.columns(2)
            save = sb.form_submit_button('Spremi izmjene')
            cancel = cb.form_submit_button('Cancel')
        if save or cancel:
            if save:
                curr = json.loads(t[7] or '[]')
                added = upload_files(st.session_state.edit_files) if st.session_state.edit_files else []
                update_tender(t[0], str(st.session_state.edit_date), st.session_state.edit_naziv, st.session_state.edit_doc, st.session_state.edit_ug, st.session_state.edit_post, st.session_state.edit_cpv, curr+added)
    ...
    vd = st.sidebar.selectbox('Vrsta dokumenta', [
        'Odaberi', 'Bagatelna nabava', 'Izmjena dokumentacije za nadmetanje',
        'Obavijest o početku postupka javne nabave', 'Obavijest o rezultatima natječaja',
        'Obavijest o sklopljenim ugovorima', 'Objava o poništenju nadметanja / ispravku objave',
        'Objava o uspostavljanju kvalifikacijskog sustava', 'Pojednostavljena obavijest u dinamičkom sustavu nabave',
        'Poziv na nadметanje', 'Poziv na natječaj', 'Prethodna (indikativna) obavijest', 'PRODAJA'
    ], key='g_vd')
    vu = st.sidebar.selectbox('Vrsta ugovora', [
        'Odaberi', 'Isporuka robe', 'Javne usluge', 'Javni radovi', 'PRODAJA'
    ], key='g_vu')
    vp = st.sidebar.selectbox('Vrsta postupka', [
        'Odaberi', 'Bagatelna nabava', 'Natječaj', 'Ograničeni postupak', 'Otvoreni postupak',
        'Pregovarački postupak bez prethodne objave', 'Pregovarački postupak s prethodном objavом',
        'PRODAJA', 'Ugovori s povezanim Društvima', 'Usluge iz Dodatka II.B'
    ], key='g_vp')
            ...
            for idx,f in enumerate(pr):
                ...
                if os.path.exists(p):
                    with open(p,'rb') as fp:
                        cols[6].download_button(label=f, data=fp.read(), file_name=f, key=f'gdl_{r[0]}_{idx}')
                else:
